...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO Session_feedback(feedback_id,session_id,client_id,rating,feedback_text) VALUES (?, ?, ?,?,?)");
    $stmt->bind_param("sssss",$feedback_id, $session_id, $client_id,$rating,$feedback_text);
...
<?php
include('databaseconnection.php');
// SQL query to fetch data from the Dog_certification
$sql = "SELECT * FROM Session_feedback";
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title> same Details of Session_feedback </title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Session_feedback</h2></center>
    <table border="5">
        <tr>
            <th>feedback_id</th>
            <th>session_id</th>
        ...
        // Prepare SQL query to retrieve all Session_feedback
        $sql = "SELECT * FROM Session_feedback";
        $result = $connection->query($sql);

        // Check if there are any Session_feedback
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_Session_feedback.php?feedback_id=$feedback_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_Session_feedback.php?feedback_id=$feedback_id'>Update</a></td> 
                </tr>";
            }
        } else {
